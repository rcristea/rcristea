...
export function generatePaths(
  ...
  const { width, height, padding, trackSpacing, branchCurveRadius, mainAxisOffset } = dimensions;

  const isHorizontal = orientation === 'horizontal';
  const mainAxisLength = isHorizontal ? width - 2 * padding : height - 2 * padding;
  const mainAxisPosition = isHorizontal ? height / 2 : (mainAxisOffset ?? width / 2);
  ...
  return events.map((event) => {
    ...
    if (isHorizontal) {
      ...
      if (event.track === 0) {
        ...
        // Branch track - 90 degree turns matching horizontal pattern:
        // main → perpendicular right to branch → down along branch → perpendicular left to main
        const branchStart = { x: mainAxisPosition, y: startPos };
        const branchEnd = { x: mainAxisPosition, y: endPos };

        d = `M ${branchStart.x} ${branchStart.y} `;
        // Horizontal right to branch track (perpendicular from main)
        d += `L ${startPoint.x} ${startPoint.y} `;
        // Vertical down along branch track
        d += `L ${endPoint.x} ${endPoint.y} `;
        // Horizontal left back to main line (perpendicular to main)
        d += `L ${branchEnd.x} ${branchEnd.y}`;
      }
    }
...
export function generateConnections(
  ...
  const { width, height, padding, trackSpacing, mainAxisOffset } = dimensions;
...
export function generateMainAxisPath(
  ...
  const { width, height, mainAxisOffset } = dimensions;
  const isHorizontal = orientation === 'horizontal';
  ...
  if (isHorizontal) {
    ...
    const x = mainAxisOffset ?? width / 2;
